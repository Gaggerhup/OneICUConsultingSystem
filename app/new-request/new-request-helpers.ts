...
export function parseDraft(rawDraft: string): RequestForm | null {
  try {
    const parsed = JSON.parse(rawDraft) as Partial<RequestForm> & { currentSymptoms?: string };
    return {
      ...initialForm,
      ...parsed,
      chiefComplaint: parsed.chiefComplaint || '',
      presentIllness: parsed.presentIllness || parsed.currentSymptoms || '',
    };
  } catch {
    return null;
  }
...
export function validateRequestForm(formData: RequestForm) {
  ...
  if (!formData.presentIllness.trim()) errors.presentIllness = 'Present illness is required';
