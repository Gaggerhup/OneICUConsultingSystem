...
import { listMonitoredCases } from '@/actions/case-monitor';
...
function Dashboard() {
  ...
  const [monitorCaseCount, setMonitorCaseCount] = useState<number | null>(null);

  useEffect(() => {
    let isMounted = true;

    listMonitoredCases(userProfile.hospital)
      .then((cases) => {
        if (isMounted) setMonitorCaseCount(cases.length);
      })
      .catch((error) => {
        console.error('[Dashboard] Unable to load case monitor count:', error);
        if (isMounted) setMonitorCaseCount(0);
      });
      ...
      <div className={styles['dashboard-content-wrapper']}>
        ...
        <div className={cx(styles, 'stats-grid', 'fade-in-float', 'delay-short')}>
          ...
          <div className={styles['stat-card']}>
            ...
            <div className={styles['stat-body']}>
              <div className={styles['stat-value']}>
                {monitorCaseCount === null ? '--' : monitorCaseCount.toString().padStart(2, '0')} <span className={cx(styles, 'stat-trend', 'positive')}>DB</span>
              </div>
              <div className={styles['stat-desc']}>
                {monitorCaseCount === null ? t('common.loading') : `${monitorCaseCount} ${t('dashboard.specialistsAcceptingCasesNow')}`}
              </div>
            </div>
          </div>
