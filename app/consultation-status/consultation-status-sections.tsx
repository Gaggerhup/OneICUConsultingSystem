        ...
        <div className={styles['patient-identity']}>
          ...
          <div className={styles['patient-text-block']}>
            ...
            <div className={styles['patient-meta-row']}>
              {patientAge} {t('consultationStatus.yearsOld')} • {patientGender} • +1 (555) 012-3456
            </div>
  ...
  return (
    <aside className={styles['medical-history-sidebar']}>
      ...
      <div className={styles['medical-data-group']}>
        ...
        <p>{t('consultationStatus.preExistingConditionsValue')}</p>
      </div>

      <div className={styles['medical-data-group']}>
        <label>{t('consultationStatus.allergies')}</label>
        <p>{t('consultationStatus.allergiesValue')}</p>
      </div>

      <div className={styles['medical-data-group']}>
        <label>{t('consultationStatus.chiefComplaint')}</label>
        <p>{reason}</p>
      </div>

      <div className={styles['initial-diagnosis-box']}>
        <label>{t('consultationStatus.initialDiagnosis')}</label>
        <h4>{t('consultationStatus.suspectedDiagnosis')}</h4>
      </div>
          ...
          <div key={member.id} className={styles['team-member-card']}>
            ...
            />
              <div className={styles['team-member-meta']}>
                ...
                <div className={styles['team-member-specialty']}>{member.specialty} • {member.role}</div>
              </div>
            </div>
        ))}
        ...
        {activeTab === 'vitals' && (
          <>
            ...
            <table className={styles['modern-labs-table']}>
              <thead>
                <tr>
                  <th>{t('consultationStatus.testLabel')}</th>
                  <th>{t('consultationStatus.resultLabel')}</th>
                  <th>{t('consultationStatus.refLabel')}</th>
                </tr>
              </thead>
              <tbody>
                <tr>
