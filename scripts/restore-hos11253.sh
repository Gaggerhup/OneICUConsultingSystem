...
SKIP_FILENAME_SUFFIXES="${SKIP_FILENAME_SUFFIXES:-log,lmage,doremon}"
...
load_source_list_file() {
  local list_file="$1"

  [[ -f "${SOURCE_LIST_FILE}" ]] || fail "Source list file not found: ${SOURCE_LIST_FILE}"

  sort -u "${SOURCE_LIST_FILE}" > "${list_file}" \
    || fail "Unable to read source list file: ${SOURCE_LIST_FILE}"
}
...
filter_skipped_sql_files() {
  local list_file="$1"
  local filtered_list
  filtered_list="$(mktemp)"

  node - "${SKIP_FILENAME_SUFFIXES}" "${list_file}" > "${filtered_list}" <<'EOF'
const fs = require('fs');

const suffixes = (process.argv[2] || '')
  .split(',')
  .map((value) => value.trim())
  .filter(Boolean);

const suffixPattern = new RegExp(`(^|[._-])(?:${suffixes.map((value) => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})$`, 'i');
...
process.stderr.write(`SKIPPED_SUFFIX_COUNT=${skipped}\n`);
process.stderr.write(`REMAINING_AFTER_SUFFIX_FILTER=${kept}\n`);
EOF
...
import_sql_file() {
  ...
  case "${SOURCE_MODE}" in
    docker)
      ...
      db-cli -g my -H "${DB_HOST}" -P "${DB_PORT}" -u "${DB_USER}" -p "${DB_PASSWORD}" -d "${DB_NAME}" -e "${sql}"
      ;;
    local)
      sql="$(iconv -f TIS620 -t UTF-8 < "${sql_file}")"
      db-cli -g my -H "${DB_HOST}" -P "${DB_PORT}" -u "${DB_USER}" -p "${DB_PASSWORD}" -d "${DB_NAME}" -e "${sql}"
      ;;
    zip)
      sql="$(
...
main() {
  ...
  if [[ -n "${SOURCE_LIST_FILE}" ]]; then
    load_source_list_file "${list_file}"
  else
    collect_sql_files "${list_file}"
  fi
  filter_skipped_sql_files "${list_file}" 2>> "${RUN_LOG}"
  ...
  if [[ -n "${SOURCE_LIST_FILE}" ]]; then
    log "SOURCE_LIST_FILE=${SOURCE_LIST_FILE}"
  fi
