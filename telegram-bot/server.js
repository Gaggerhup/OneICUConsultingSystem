...
// Initialize the Telegram bot with polling so it can receive messages
console.log('[Debug] Step 8: Initializing Telegram Bot...');
const bot = new TelegramBot(token, { polling: true });

// Add listener for /start command so users can find their Chat ID
bot.onText(/\/(start|chatid)/, (msg) => {
  const chatId = msg.chat.id;
  const username = msg.from.username ? `@${msg.from.username}` : msg.from.first_name;
  
  bot.sendMessage(chatId, `สวัสดีคุณ ${username} 👋\n\nTelegram Chat ID ของคุณคือ:\n<code>${chatId}</code>\n\nคุณสามารถคัดลอกตัวเลขนี้ไปใส่ในเมนู Settings > Notifications บนระบบ OneICU เพื่อรับการแจ้งเตือนส่วนตัวได้เลยครับ`, {
    parse_mode: 'HTML'
  });
});
...
app.post('/register', (req, res) => {
    ...
    specialistRegistry[specialistId] = {
        ...
        preferences: Array.isArray(preferences) ? preferences : ['newRequest', 'requestApproved', 'newMessage', 'caseUpdate', 'systemAlert'], // Default set
        lastSeen: Date.now()
    };
