...
export const CENTRAL_MONITOR_HOSPITALS = [
...
export const canAccessAllHospitals = (hospital: string | null | undefined) =>
  CENTRAL_MONITOR_HOSPITALS.includes(normalizeHospitalName(hospital) as typeof CENTRAL_MONITOR_HOSPITALS[number]);

export const canAccessHospital = (
  userHospital: string | null | undefined,
  targetHospital: string | null | undefined,
) => {
  if (canAccessAllHospitals(userHospital)) return true;

  const normalizedUserHospital = normalizeHospitalName(userHospital);
  const normalizedTargetHospital = normalizeHospitalName(targetHospital);
  return Boolean(normalizedUserHospital && normalizedTargetHospital && normalizedUserHospital === normalizedTargetHospital);
};
...
export const mergeLockedProviderIdentity = <T extends Record<string, any>>(
  ...
  return {
    ...
    hospital: identity.hospital || profile.hospital,
