...
export async function listCasesWithOverrides() {
  ...
  return rows.map((row) => ({
    ...
    requestedAt: row.record_date
      ? `${dateToString(row.record_date)}T${row.record_time || '00:00:00'}`
      : null,
    ...
    presentIllness: row.current_symptoms,
...
export async function patchCase(caseId: string, patch: Record<string, unknown>) {
  ...
  const fieldMap: Record<string, string> = {
    ...
    presentIllness: 'current_symptoms',
...
export async function createRequestAgainFromStoredCase(caseId: string) {
  const sourceCaseId = toCaseRegisterId(caseId);
  const rows = await query<any>(`
    SELECT
      patient_id, an, hospital, priority, specialty, reason, current_symptoms,
      initial_diagnosis, clinical_notes, sender_id
    FROM case_register
    WHERE id = ?
    LIMIT 1
  `, [sourceCaseId]);
  ...
  const source = rows[0];
  if (!source) throw new Error('Archived case not found');

  const now = splitDateTime();
  await execute(`
    UPDATE case_register
    SET an = ?, hospital = ?, record_date = ?, record_time = ?, status = ?, priority = ?, specialty = ?, reason = ?,
        current_symptoms = ?, initial_diagnosis = ?, clinical_notes = ?, sender_id = ?, last_action = ?, last_active_time = ?
    WHERE id = ?
  `, [
    source.an || null,
    source.hospital || null,
    now.date,
    now.time,
    'Pending',
    source.priority || 'NON-URGENT',
    source.specialty || null,
    source.reason || null,
    source.current_symptoms || null,
    source.initial_diagnosis || null,
    source.clinical_notes || null,
    source.sender_id || null,
    'Requested again',
    new Date().toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }),
    sourceCaseId,
  ]);
  await recordCaseWorkflowEpisode({
    caseId: String(sourceCaseId),
    patientId: source.patient_id,
    episodeType: 'request',
    status: 'Pending',
    action: 'Requested again',
    actorId: source.sender_id || null,
    note: 'Request again from archived request',
  }).catch((error) => console.error('[patient-detail-store] Unable to record workflow episode:', error));

  return getCaseWithOverrides(String(sourceCaseId));
}
...
export async function upsertStoredCase(input: UpsertStoredCaseInput) {
  ...
  if (existingCaseId) {
    ...
    `, [
      ...
      input.chiefComplaint || input.reason || null,
      input.presentIllness || input.currentSymptoms || null,
