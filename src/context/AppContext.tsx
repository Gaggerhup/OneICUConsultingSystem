...
const isLocalOnlyCase = (id: string) => !/^\d+$/.test(id);
  ...
  const refreshFromStorage = useCallback(async () => {
    ...
    try {
      ...
      if (dbCases.length > 0) {
        ...
        const localRequestCases = storedRequests.filter((item) => isLocalOnlyCase(item.id));
        const localActiveCases = storedActiveCases.filter((item) => isLocalOnlyCase(item.id));
        const localArchiveCases = storedArchiveCases.filter((item) => isLocalOnlyCase(item.id));

        nextRequests = [...localRequestCases, ...normalizedCases.filter((item) => isRequestStatus(item.status))];
        nextArchiveCases = [...localArchiveCases, ...normalizedCases.filter((item) => isArchiveStatus(item.status))];
        nextActiveCases = [...localActiveCases, ...normalizedCases.filter((item) => !isRequestStatus(item.status) && !isArchiveStatus(item.status))];
      }
