...
const resolveRedirectUri = (override?: string) => {
  ...
  return REDIRECT_URI || `${resolvePublicAppUrl()}${CALLBACK_PATH}`;
};
...
export const authService = {
  ...
  /**
   * Register the current browser with the server-side session store.
   * The API overwrites the fallback auth_session cookie with an httpOnly
   * session id that can be listed and revoked from other devices.
   */
  registerServerSession: async (profile: any, options?: { isMock?: boolean }) => {
      ...
      if (session) {
        ...
        return identity
          ? {
              ...
              hospital: normalizeHospitalName(parsedSaved.hospital) || identity.hospital,
            }
          : parsedSaved;
      }
      return parsedSaved;
  ...
  setupMockSession: async () => {
    ...
    await authService.registerServerSession(mockProfile, { isMock: true });
