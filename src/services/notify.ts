...
export async function dispatchNotification(params: DispatchParams): Promise<void> {
  ...
  try {
    // 1. Query all providers who accept notifications
    const allProviders = await db
      .select()
      .from(users)
      .where(eq(users.isAcceptingNotifications, true));
    ...
    console.log(
      `[Notify] Event "${eventType}" → ${recipients.length}/${allProviders.length} recipients`,
    );
    ...
    if (recipients.length > 0) {
      const rows = recipients.map((provider) => ({
        userId: provider.id,
        notifyDate,
        notifyTime,
        title,
    ...
    if (defaultChatId && !sentChats.has(defaultChatId)) {
